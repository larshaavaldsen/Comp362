...
void step2Safety(bool *finish, double *work) {
    for (int i = 0; i < numOfCustomers; i++) {
        if(finish[i] == false && lessOrSame(need[i], work, numOfAccounts)) {
            add(work, allocation[i],  numOfAccounts);
            finish[i] = true;
            step2Safety(finish, work);
        }
    }
...
{   
    // step 1 - Work = Available Finish[i] = false for i = 0, 1, …, n-1
    double *work = malloc(sizeof(double) * numOfAccounts);
    ...
    return allTrue(finish, numOfAccounts);
}

/***
 * Determines whether a request for loan can be granted.
 check that we can grant the loan - 
 */
bool solvencyTest(int customerNum, double *request)
{   
    // step 1 - If Request[i] ≤ Need[i] go to step 2. Otherwise, raise error 
    // condition, since process Pi has exceeded its maximum claim.
    // use less than or same array func, if true return false on request, need
    ...
    // step 2 - If Request[i] ≤ Available, go to step 3. Otherwise process Pi must wait, 
    // since the resources that it needs are not available
    // use less than or same func on request, availble
    for (int i = 0; i < numOfAccounts; i++) {
    ...
    // step 3 - 
    return isSafeState(customerNum, request); // TODO: modify as appropriate
}

/***
 * Make a request for a loan.
 first check safe state
 second check solvency
 if solvency is okay, process loan
 */
